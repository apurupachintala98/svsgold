...
export default function ApplicationForm({
  ...
  /*  1. localStorage (saved from /accounts/create + /addresses)      */
  /*  2. getFinalPreview (if user has prior applications)             */
  /*  3. checkAccount fallback                                        */
  /* ================================================================ */
  useEffect(() => {
    ...
    const fetchCustomerInfo = async () => {
      ...
      // === Source 1: localStorage (exact data from Create Account form) ===
      try {
        const stored = JSON.parse(localStorage.getItem('svs_customer_info') || '{}')
        if (stored.name) fullName = stored.name
        if (!fullName && (stored.first_name || stored.last_name)) {
          fullName = [stored.first_name, stored.last_name].filter(Boolean).join(' ')
        }
        if (stored.addresses && Array.isArray(stored.addresses) && stored.addresses.length > 0) {
      ...
      // === Source 2: getFinalPreview (works if prior applications exist) ===
      if (!fullName) {
        try {
          const res = await applicationsAPI.getFinalPreview(userIdentifier)
          const data = res.data || {}
          const account = data.account || data.customer || {}
          const addresses = data.addresses || []

          fullName = account.name || [account.first_name, account.last_name].filter(Boolean).join(' ') || ''
          if (!fullName && data.pledge_details?.pledger_name) fullName = data.pledge_details.pledger_name

          if (!fullAddress && addresses.length > 0) {
            const a = addresses.find(a => /present|current|residential/i.test(a.address_type)) || addresses[0]
            fullAddress = [a.address_line, a.street, a.city, a.state, a.pincode].filter(Boolean).join(', ')
          }
          if (!fullAddress && data.pledge_details?.pledger_address) fullAddress = data.pledge_details.pledger_address
          if (!fullAddress) fullAddress = [account.address_text, account.city, account.state, account.pincode].filter(Boolean).join(', ')
        } catch (e) { /* no preview data */ }
      }

      // === Source 3: checkAccount (direct account lookup) ===
      if (!fullName) {
        try {
          const res = await accountsAPI.checkAccount({ mobile: userIdentifier })
          const acc = res.data?.account || res.data || {}
          fullName = acc.name || [acc.first_name, acc.last_name].filter(Boolean).join(' ') || ''
          if (!fullAddress) fullAddress = [acc.address_text, acc.city, acc.state, acc.pincode].filter(Boolean).join(', ')
        } catch (e) { /* no account data */ }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select value={pledgeDetails.branch_name} onChange={(e) => handlePledgeChange('branch_name', e.target.value)} className={inputClass}>
            <option value="">Select Branch</option>
            <option value="Dilsukhnagar">Dilsukhnagar</option>
            <option value="Narayanguda">Narayanguda</option>
          </select>
